...
export default function App() {
  ...
  const [goals,      setGoals]      = useState({ age: "", gender: "male", height: "", weight: "", targetWeight: "", fitnessGoal: "lean", activityLevel: "moderate", dietaryPrefs: "", timeframe: "12" });
  ...
  const handlePhoto = (e) => {
    ...
    r.onload = async (ev) => {
      const resized = await resizeImg(ev.target.result, 900);
      setPhoto(resized); setPhotoB64(resized.split(",")[1]); setPhotoMime("image/jpeg");
    };
    r.readAsDataURL(f);
  };
  ...
  const handleProgPhoto = (e) => {
    ...
    const r = new FileReader();
    r.onload = async (ev) => {
      const resized = await resizeImg(ev.target.result, 480);
      const entry = { id: Date.now(), dataUrl: resized, label: `Week ${week}`, date: new Date().toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) };
      ...
      {step === 2 && (
        <div style={{ padding: "2rem 1.5rem 0", maxWidth: 560 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: "1.25rem" }}>
            <div><div style={lbl}>Gender</div><select style={{ ...inp, appearance: "none" }} value={goals.gender} onChange={e => setG("gender", e.target.value)}><option value="male">Male</option><option value="female">Female</option><option value="other">Other</option></select></div>
          ...
          <div style={{ marginBottom: "1.25rem" }}>
            <div style={lbl}>Dietary preferences / restrictions</div>
            <input style={inp} type="text" placeholder="e.g. vegetarian, no gluten, intermittent fasting…" value={goals.dietaryPrefs} onChange={e => setG("dietaryPrefs", e.target.value)} />
          </div>
        ...
        <div style={{ padding: "5rem 1.5rem 2rem", maxWidth: 480 }}>
          ...
          {LOAD_STEPS.map((ls, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 14, padding: "0.85rem 1rem", background: i === lStep ? "#1A1D1E" : "transparent", borderRadius: 10, marginBottom: 4, transition: "all 0.4s" }}>
              <div style={{ width: 10, height: 10, borderRadius: "50%", flexShrink: 0, background: i < lStep ? A : i === lStep ? A : "#2A2D30", boxShadow: i === lStep ? `0 0 10px ${A}` : "none" }} />
